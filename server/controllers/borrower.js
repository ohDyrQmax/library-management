...
export const borrowerRegister = async (req, res) => {
  try {
    let { username, password, name } = req.body || req;
...
export const borrowerLogin = async (req, res) => {
  try {
    ...
    let token = jwt.sign({ id: borrower.__id }, process.env.JWT_SECRET);
...
/* read */
export const getBorrowerDetail = async (req, res) => {
  try {
    let { id } = req.params || req;
    let borrower = Borrower.findById(id);
